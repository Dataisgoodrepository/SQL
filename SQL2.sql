...
--	Update an Employees Salary Suppose you want to give a raise to Alice Johnson, increasing her salary to  $80,000:
...
--	Updating Multiple Columns You can also update multiple columns at once. For example, if you want to update both the job title and salary for David Brown:
...
-- SELECT Command with != or <>(Not equal to), >(Greater than),< (Less than),>= (Greater than or equal to) & <= (Less than or equal to):
...
-- The BETWEEN operator is used to filter results within a specified range, inclusive of the boundary values. It allows you to easily retrieve records where a column's value falls between two specified values, making it useful for analysing ranges in numerical or date data.
...
-- The LIKE operator is used to search for a specified pattern in a column, allowing for flexible matching of text. It is often used with wildcard characters:
...
-- Using And Operator:
--	Finding Employees with a Specific Job Title and Salary
